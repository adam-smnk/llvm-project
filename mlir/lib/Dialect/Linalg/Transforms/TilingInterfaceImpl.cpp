...
/// Verify that tile sizes for `ScaledContractOp` are compatible with the
/// scaling scheme encoded in the scale indexing maps.
static LogicalResult
verifyScaledContractTileSizes(linalg::ScaledContractOp scaledContractOp,
  ...
  // Scale maps are at fixed positions: index 1 (lhs scale) and 3 (rhs scale).
...
/// scheme are compatible with tile sizes.
template <>
FailureOr<TilingResult>
